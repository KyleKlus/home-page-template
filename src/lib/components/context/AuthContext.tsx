...
import firebase_auth from '@/lib/services/firebaseAuth';
import { initFirebase } from '@/lib/services/firebase';
...
export const AuthContext: React.Context<IAuthContext> = createContext<IAuthContext>(defaultValue);
initFirebase();
...
const AuthProvider: React.FC<{ children: React.ReactNode }> = (props) => {
    ...
    useEffect(() => {
        if (!loading && !user) router.push(redirectPaths[RedirectPathOptions.LoginPage]);
    }, [user, loading]);
